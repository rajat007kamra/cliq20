//package actions.login;
//
//import java.util.Date;
//import java.util.List;
//
//import org.apache.log4j.Logger;
//import org.openqa.selenium.WebDriver;
//import org.openqa.selenium.WebElement;
//import org.testng.Assert;
//
//import com.google.gson.Gson;
//
//import actions.Actions;
//import actions.PageHelper;
//import actions.model.ForgotPasswordModel;
//import common.variables.CommonVariables;
//import io.github.sukgu.Shadow;
//import selenium.driver.Driver;
//
//
//public class ForgotPassword extends Driver implements Actions {
//
//	private WebDriver driver;
//	private ForgotPasswordModel forgotModel;
//	private Shadow shadow;
//	
//	public static String forgot = "span.content-link.flt-rght";	
//	public static String securityanswer = "input.mdc-text-field__input";
//	public static String send_button = "mz-button#submitButton.flt-rght.button-main";
//	public static String message = "span.error-text";
//	
//	final static Logger logger = Logger.getLogger(ForgotPassword.class);
//	public ForgotPassword(WebDriver driver, String jsonString) {
//		super(driver);
//		this.driver = driver;
//		shadow = new Shadow(driver);
//		this.forgotModel = new Gson().fromJson(jsonString, ForgotPasswordModel.class);
//	}
//
//	@Override
//	public Date execute() {
//		try {
//			PageHelper.waitInSeconds(this.driver, PageHelper.XX_TIMEOUT_SEC);
//			forgotpassword(this.forgotModel.getusername(), this.forgotModel.getanswer());
//			return null;
//		} catch (Exception e) {
//			e.printStackTrace();
//			Assert.assertTrue(false, e.getMessage());
//			return null;
//		}
//	}
//
//	@Override
//	public Date validate() {
//		return null;
//	}
//
//	@Override
//	public Date setup() {
//		return null;
//	}
//
//	@Override
//	public Date tearDown() {
//		return null;
//	}
//
//	public void forgotpassword(String username, String answer) throws InterruptedException {
//		WebElement forgot = shadow.findElement(forgot);
//		List<WebElement> element = shadow.findElements("input.mdc-text-field__input");
//		
//		if (this.forgotModel.getusername() != null) {
//			if (PageHelper.isElementDisplayed(forgot)) {
//				PageHelper.sendKeys(shadow, element.get(0), username);
//				PageHelper.click(shadow, forgot);
//				logger.info("Click on Forgot Password ");
//				WebElement securityanswer = shadow.findElement(securityanswer);
//				PageHelper.sendKeys(shadow, securityanswer, answer);
//				logger.info("Security answer is " + answer);
//				WebElement sendbutton = shadow.findElement(send_button);
//				PageHelper.click(shadow, sendbutton);
//				Thread.sleep(10000);
//				WebElement message = shadow.findElement(message);
//				logger.info("Submit button clicked");
//				String msg = PageHelper.getText(shadow, message);
//				if (msg.equals(this.forgotModel.getanswer())) {
//					logger.info("UNKNOWN Error, Incorrect Answer");
//				} else {
//					logger.info(msg);
//				}
//			} else {
//				logger.info("Forgot Password Buuton Not Found");
//			}
//		} else {
//			logger.info("Either Username is Invalid or Empty");
//		}
//	}
//}
