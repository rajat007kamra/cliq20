...
/**
 * @summary This class contains verify, click, and validate Publish icon
 * @author Arun.Kapoor
 */

public class Publish implements ViewZone {
	private WebDriver driver;
	final static Logger logger = Logger.getLogger(RecordDetails.class);
	Shadow shadow;

	public Publish(WebDriver driver,String jsonString) {
		this.driver = driver;
	...
	private String buttonName = "PUBLISH";
	
	@Override
	public Date execute() {
		try {
			verifyIsButtonDisabled(buttonName);
			clickPublishButton(this.driver, buttonName);
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * @summary This method verifies whether Publish icon is disabled or enabled in viewzone  
	 * @param Button_Action
 	 * @return
	 */
	...
	/**
	 * @summary This method use to click on Publish icon 
	 * @param driver, buttonAction
 	 * @return
	 */
	@Step("Clicked at button name {buttonName}")
	public void clickPublishButton(WebDriver driver, String buttonName)
	{
