...
/**
 * @summary Handles Discard Button
 * @author Rajat.Kamra
 *
 */
public class Discard implements EditZone {
	private WebDriver driver;
	final static Logger logger = Logger.getLogger(Discard.class);
	private Shadow shadow;
	private String disabledIcons = "mz-standard-realm[active] mz-icon-label-button[text='Discard'] div.icon-label-button--disabled";
	private String discardIcon="mz-standard-realm[active] mz-icon-label-button[text='Discard'] div.icon-label-button";
	...
	public Date execute() {
		try {
			checkIfIconIsDisabled(this.driver,"DISCARD");
			if(shadow.findElements(discardIcon).size()>0)
            {
                logger.info("Discard Found");
