...
/**
 * @summary Handles Save button
 * @author Rajat.Kamra
 *
 */
public class Save implements EditZone {
	private WebDriver driver;
	final static Logger logger = Logger.getLogger(Save.class);
	private Shadow shadow;
	private String disabledIcons = "mz-standard-realm[active] mz-icon-label-button[text='Save'] div.icon-label-button--disabled";
	private String saveIcon="mz-standard-realm[active] mz-icon-label-button[text='Save'] div.icon-label-button";
	...
	public Date execute() {
		try {
			...
			if(shadow.findElements(saveIcon).size()>0)
            {
