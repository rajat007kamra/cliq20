 ...
 *
 */
public class Submit implements EditZone {
	private WebDriver driver;
	final static Logger logger = Logger.getLogger(Submit.class);
	private Shadow shadow;
	private String disabledIcons = "mz-standard-realm[active] mz-icon-label-button[text='Submit'] div.icon-label-button--disabled";
	private String submitIcon="mz-standard-realm[active] mz-icon-label-button[text='Submit'] div.icon-label-button";
	...
	public Date execute() {
		try {
			...
			if(shadow.findElements(submitIcon).size()>0)
            {
                logger.info("Submit Found");
                //shadow.findElement(submitIcon).click();
            }
			} catch (Exception e) {
			e.printStackTrace();
